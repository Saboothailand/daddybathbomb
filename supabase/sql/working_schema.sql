...
-- 3. 실제 제품 데이터 삽입 (태국 바트)
INSERT INTO public.products (name, description, short_description, price, original_price, image, sku, stock_quantity, is_featured, is_active, category, scent, weight, ingredients, color, rating, review_count) 
SELECT 'Strawberry Splash Bomb', '싱싱한 딸기 향이 폭발하는 버블 배쓰밤', '딸기향 버블과 핑크빛 거품', 390.00, 450.00, 'https://images.unsplash.com/photo-1585325701961-89c09fd9b1f7?w=500&h=400&fit=crop', 'DBB-FRUIT-001', 50, true, true, 'Fruit Splash', 'Fresh Strawberry', '120g', 'Strawberry extract, coconut oil, shea butter', '#ff5b7f', 4.8, 126
WHERE NOT EXISTS (SELECT 1 FROM public.products WHERE name = 'Strawberry Splash Bomb');

INSERT INTO public.products (name, description, short_description, price, original_price, image, sku, stock_quantity, is_featured, is_active, category, scent, weight, ingredients, color, rating, review_count) 
SELECT 'Mango Tango Fizzy Bomb', '열대 망고 향이 가득한 트로피컬 버블', '망고와 코코넛의 달콤한 조화', 420.00, 480.00, 'https://images.unsplash.com/photo-1514996937319-344454492b37?w=500&h=400&fit=crop', 'DBB-FRUIT-002', 45, true, true, 'Fruit Splash', 'Tropical Mango', '125g', 'Mango butter, coconut milk powder, sea salt', '#ffa726', 4.7, 98
WHERE NOT EXISTS (SELECT 1 FROM public.products WHERE name = 'Mango Tango Fizzy Bomb');

INSERT INTO public.products (name, description, short_description, price, original_price, image, sku, stock_quantity, is_featured, is_active, category, scent, weight, ingredients, color, rating, review_count) 
SELECT 'Pineapple Paradise Soak', '파인애플과 라임이 어우러진 상큼 버블', '파인애플 향 폭발', 380.00, 430.00, 'https://images.unsplash.com/photo-1524592094714-0f0654e20314?w=500&h=400&fit=crop', 'DBB-FRUIT-003', 40, true, true, 'Fruit Splash', 'Pineapple Lime', '115g', 'Pineapple extract, lime oil, baking soda', '#ffd54f', 4.6, 84
WHERE NOT EXISTS (SELECT 1 FROM public.products WHERE name = 'Pineapple Paradise Soak');

INSERT INTO public.products (name, description, short_description, price, original_price, image, sku, stock_quantity, is_featured, is_active, category, scent, weight, ingredients, color, rating, review_count) 
SELECT 'Blueberry Bliss Bomb', '진한 블루베리 향의 딥블루 버블', '베리향 릴랙스 타임', 410.00, 470.00, 'https://images.unsplash.com/photo-1593251698860-4fcc1e2392b9?w=500&h=400&fit=crop', 'DBB-FRUIT-004', 38, true, true, 'Fruit Splash', 'Blueberry Vanilla', '130g', 'Blueberry extract, vanilla bean, almond oil', '#5c6bc0', 4.9, 154
WHERE NOT EXISTS (SELECT 1 FROM public.products WHERE name = 'Blueberry Bliss Bomb');

INSERT INTO public.products (name, description, short_description, price, original_price, image, sku, stock_quantity, is_featured, is_active, category, scent, weight, ingredients, color, rating, review_count) 
SELECT 'Watermelon Wave Soother', '수박 주스 같은 청량 버블', '한여름 수박 향기', 360.00, 410.00, 'https://images.unsplash.com/photo-1601004890684-d8cbf643f5f2?w=500&h=400&fit=crop', 'DBB-FRUIT-005', 48, true, true, 'Fruit Splash', 'Watermelon Mint', '120g', 'Watermelon extract, mint oil, aloe powder', '#ff6f91', 4.4, 61
